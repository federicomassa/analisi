...
void readfile() {
 ...
 TH1F* posx_dist = new TH1F("posx_dist","Distribuzione delle entries x; X; Conteggio",200,1,51);
 TH1F* posy_dist = new TH1F("posy_dist","Distribuzione delle entries y; Y; Conteggio",200,1,51); 
 TH1F* posxy_dist = new TH1F("posxy_dist","Distribuzione delle entries y; Y; Conteggio",200,0,50);  
  radius0_dist->GetYaxis()->SetTitleOffset(1.4);
  TFile* out = new TFile("readfile2.root","RECREATE");
  ...
  do{
    ...
    if (i >= 3) { // inizio eventi
      posx_dist->Fill(pxx(strnum));
      posy_dist->Fill(pxy(strnum));
      posxy_dist->Fill(TMath::Sqrt((pxx(strnum)-24.5)*(pxx(strnum)-24.5)+(pxy(strnum)-24.5)*(pxy(strnum)-24.5)));
    }
 ...
 TH1F* entries_dist = new TH1F("entries_dist","Distribuzione del numero di eventi generati;Eventi;Conteggio",200,49000,51000);
  ...
  for (int k = 0; k < kmax; k++) {
    ...
    integral = (entries - TMath::Sqrt(8*TMath::ATan(1))*gausfit->GetParameter(3)*gausfit->GetParameter(5)/8E-3)/entries;
